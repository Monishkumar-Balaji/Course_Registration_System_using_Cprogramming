...
void Drop_Course(int *Student_Id)
{
    ...
    {
        int num_courses;
        printf("Enter Number of Courses to Drop (Minimum number of courses should be 3): ");
        scanf("%d", &num_courses);
        int eligible = Check_Eligibility();
        if (eligible==1)
        {
        ...
        {
            printf("\nSorry, You are late for dropping courses as the time duration was within two weeks.\n");
            printf("Now the registration has been closed. \n\n");
        }
        fclose(userfile);
    }
    else if (strcmp(status, "YES") == 0) 
...
void writeWaitlist(int *Student_Id, const char *faculty, const char *course_code, char type) {
    ...
    FILE *fp_backlog = fopen("backlog_courses.bin", "rb");
    if (fp_backlog == NULL) {
